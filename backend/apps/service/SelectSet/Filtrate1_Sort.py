...
class Filtrate1Sort(SparkSessionBase):
    SPARK_URL = "local"
    ...
    # (1,2)
    def choose_two(self, state, s1, s2):
        hc = HiveContext(self.spark.sparkContext)
        b_df = hc.table('business')
        r_df = hc.table('review')
        five_df = r_df.where(col('rev_stars') == 5) \
    ...
    # (1,3)
    def choose_three(self, state, s1, s2, s3):
        hc = HiveContext(self.spark.sparkContext)
        b_df = hc.table('business')
        r_df = hc.table('review')
        five_df = r_df.where(col('rev_stars') == 5) \
